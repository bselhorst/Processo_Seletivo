...
<div class="card">
    ...
    <div class="card border-top border-top-width-1 border-bottom border-bottom-width-1 rounded-0" style="margin: 10px">
        ...
        <form class="needs-validation" method='POST' action="{{ @$data_user ? route('usuarios.update', $data_user->id) : route('usuarios.store') }}" enctype='multipart/form-data' novalidate>
            @csrf
            @if (@$data_user)
                @method('patch')
            @endif
            <div class="card" style="padding-top: 20px">           
                ...
                <div class="row mb-3">
                    <label class="col-form-label col-lg-2 offset-lg-1">Super Admin?</label>
                    {{-- <div class="col-lg-8">
                        <input type="text" name="name" class="form-control" placeholder="Nome completo" value="{{ (@$data_user->name) ? $data_user->name : '' }}" required>
                        <div class="invalid-feedback">Campo obrigatório!</div>
                        <div class="valid-feedback">Campo válido</div>
                    </div> --}}
                    <div class="col-lg-2">
                        <select class="form-control select" name="isSuperAdmin" aria-hidden="true">
                            <option value="0">Não</option>
                            <option value="1" {{ (@$data_user->isSuperAdmin == 1) ? 'selected' : '' }}>Sim</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    <label class="col-form-label col-lg-2 offset-lg-1">Email</label>
                    <div class="col-lg-8">
                        <input type="text" name="email" class="form-control" placeholder="Email" value="{{ (@$data_user->email) ? $data_user->email : '' }}">
                        <div class="invalid-feedback">Campo obrigatório!</div>
                        <div class="valid-feedback">Campo válido</div>
                    </div>
                </div>
                ...
                <div class="card-footer text-end">
                    @if (@$data_user)
                        <a href="{{ route('usuarios.index') }}" class="btn btn-outline-primary">Cancelar </a>
    ...
    {{-- <div class="card border-top border-top-width-1 border-bottom border-bottom-width-1 rounded-0" style="margin: 10px">
        ...
        <form method="POST" action="{{ route('usuarios.indexSearch') }}">
            @csrf
            ...
            <tbody>
                @foreach ($data as $item)
                    <tr>
                        ...
                        <td>{{ $item->name }}</td>
                        <td>{{ $item->email }}</td>
                        <td>{{ ($item->isSuperAdmin == 1) ? 'SIM' : 'NÃO' }}</td>
                        <td class="text-center">
                            <div class="d-inline-flex">
                                <div class="dropdown" {{(Auth::user()->id == $item->id) ? 'hidden' : ''}}>
                                    <a href="#" class="text-body" data-bs-toggle="dropdown">
                                        <i class="ph-list"></i>
                                    </a>
                                    <div class="dropdown-menu dropdown-menu-end">
                                        <a href="{{ route('usuarios.edit', $item->id) }}" class="dropdown-item">
                                            <i class="ph-note-pencil me-2"></i>
                                            Editar Usuário
                                        </a>
                                        <form id="delete_form_{{$item->id}}" method="POST" action="{{ route('usuarios.destroy', $item->id) }}">
                                            @csrf
                                            @method('delete')
                                            ...
                                            <button type="button" class="dropdown-item" onclick="deleteForm({{ $item->id }})">
                                                ...
                                                <div style="padding-left: 10px" class="text-danger">Excluir Usuário</div> 
                                            </button>
                                        </form>
                                    </div>
                                </div>
